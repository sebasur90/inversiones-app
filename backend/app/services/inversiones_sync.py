...
def sync_from_sheet(db: Session) -> dict:
    ...
    try:
        ...
        # Red de seguridad: si el cuerpo lanzó antes del finalizar_corrida() de arriba
        # (p.ej. SheetsClientError al leer el Sheet), sin esto el contador quedaría
        # activa=True y arrastraría su tope por-corrida al próximo sync. Es idempotente.
        iol_auth.finalizar_corrida()
