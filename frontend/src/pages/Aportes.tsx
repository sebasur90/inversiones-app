...
import AportesMensualesChart from '../components/charts/AportesMensualesChart'
import AportesHeatmap from '../components/charts/AportesHeatmap'
...
function mesCorto(mes: string | null | undefined): string {
  ...
  return (
    <div className="mb-2">
      ...
      <div className="h-1.5 rounded-full bg-app-surface-2 overflow-hidden">
        <div className="h-full rounded-full bg-app-accent" style={{ width: `${hito.progreso_pct}%` }} />
      </div>
    </div>
  )
}
...
function Contenido({ datos, cartera }: { datos: RitmoAportesOut; cartera: string | null }) {
  ...
  return (
    <>
      <div className="text-label text-app-text-dim mb-3 flex items-center gap-1">
        Métricas en USD (dólar MEP del día de cada movimiento)
      ...
      <Card className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-body font-bold text-app-text">
            <InfoTooltip term="aportes_hitos" label="Logros" />
          </h3>
          <button
        ...
        <div className="flex flex-col gap-2.5">
          {anio.proyecciones.map(p => (
            <div key={p.clave}>
              ...
              <div className="h-1.5 rounded-full bg-app-surface-2 overflow-hidden">
                <div
                  className={`h-full rounded-full ${(p.total_fin_anio_usd ?? 0) < 0 ? 'bg-app-neg' : 'bg-app-accent'}`}
                  style={{ width: `${(Math.abs(p.total_fin_anio_usd ?? 0) / maxProyeccion) * 100}%` }}
                />
              </div>
            </div>
          ))}
...
export default function Aportes() {
  ...
  if (!datos || datos.estado === 'sin_datos' || !datos.este_mes) {
    return (
      <div className="pb-4">
        <ScreenHeader title="Ritmo de aportes" onBack={() => navigate(-1)} />
        <EmptyState
          title="Sin aportes registrados"
          description="Cuando registres compras o ventas, acá vas a ver tu ritmo mes a mes, rachas y proyección a fin de año."
        />
      </div>
    )
  }
  ...
  return (
    <div className="pb-4">
      <ScreenHeader title="Ritmo de aportes" onBack={() => navigate(-1)} />
      <Contenido datos={datos} cartera={carteraSeleccionada} />
    </div>
  )
}
