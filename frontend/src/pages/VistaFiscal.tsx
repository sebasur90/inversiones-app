...
export default function VistaFiscal() {
  ...
  const fiscalQuery = useQuery({
    queryKey: qk.de('vista-fiscal', carteraSeleccionada),
    queryFn: () => getVistaFiscalPorAnio(carteraSeleccionada),
  })
  ...
  if (fiscalQuery.isLoading || fiscalQuery.error) {
    return (
      <div className="pb-4">
        <ScreenHeader title="Vista fiscal" onBack={() => navigate(-1)} />
        <QueryBoundary
          isLoading={fiscalQuery.isLoading}
          error={fiscalQuery.error}
          onRetry={() => void fiscalQuery.refetch()}
  ...
  return (
    <div className="pb-4">
      ...
      <Card className="mb-3">
        ...
        <div className={`font-mono text-metric-lg font-bold tabular-nums ${toneClass(totResultado)}`}>
          {fmt(totResultado)}
        </div>
        <div className="text-label text-app-text-dim mt-1 tabular-nums">
            ...
            <Card key={a.anio} className="!p-0 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-strong font-bold text-app-text tabular-nums">{a.anio}</span>
                </div>
                <div className={`font-mono font-bold text-strong tabular-nums ${toneClass(resultado)}`}>
                  {fmt(resultado)}
                </div>
              </button>
              ...
              <div className="grid grid-cols-3 gap-2 px-4 pb-3 text-center">
                <div>
                  <div className="text-label font-bold uppercase tracking-wide text-app-text-faint">Realizado</div>
                  <div className={`font-mono text-caption font-semibold tabular-nums ${toneClass(realizado)}`}>{fmt(realizado)}</div>
                </div>
                <div>
                  <div className="text-label font-bold uppercase tracking-wide text-app-text-faint">Ingresos</div>
                  <div className="font-mono text-caption font-semibold tabular-nums text-app-text">{fmt(ingresos)}</div>
                </div>
                <div>
                  <div className="text-label font-bold uppercase tracking-wide text-app-text-faint">Comisiones</div>
                  <div className="font-mono text-caption font-semibold tabular-nums text-app-text-dim">{fmt(comisiones)}</div>
                </div>
              </div>

              {abierto && (
                <div className="border-t border-app-border">
                  <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-3 gap-y-1.5 px-4 py-3 text-label">
                    <div className="text-app-text-faint font-bold uppercase tracking-wide text-label">Ticker</div>
                    <div className="text-app-text-faint font-bold uppercase tracking-wide text-label text-right">Realizado</div>
                    <div className="text-app-text-faint font-bold uppercase tracking-wide text-label text-right">Ingresos</div>
                    <div className="text-app-text-faint font-bold uppercase tracking-wide text-label text-right">Comis.</div>
                    {a.por_ticker.map(t => {
                      ...
                      return (
                        <div key={t.ticker} className="contents">
                          ...
                          >
                            ...
                            <div className="text-label text-app-text-dim truncate">{t.nombre}</div>
                          </button>
      ...
      <p className="text-label text-app-text-dim mt-3 leading-relaxed">
