...
6. ~~**Costo real de operar**~~ — ✅ HECHO. El TWR encadenado ya venía **neto** de comisiones
   (los cashflows usan `_monto_ajustado` = bruto ± comisión; las valuaciones son a precio de
   mercado, sin comisión → cada comisión es un drag inmediato de `-K/v0` en su sub-período).
   Se agregó la variante **bruta**: `_monto_usd_bruto`/`_monto_ars_bruto` (= `_monto_bruto`
   convertido, sin comisión) + `_calcular_twr_bruto`/`_calcular_twr_ars_bruto` (reusan
   `_calcular_twr_encadenado`, sólo cambian la función de monto). `_resumen_sobre_movs` expone
   `twr_usd_bruto` y `twr_ars_bruto` (Optional, backward-compatible; también quedan en las
   posiciones por ticker vía `_resumen_sobre_movs`). Frontend: dos filas nuevas en la tabla
   "Rendimiento: nominal vs. real vs. USD" de `Rendimiento.tsx` — **"TWRR sin comisiones"** y
...
11. ~~**Exposición y concentración por país**~~ — ✅ HECHO. `get_exposicion` **ya** tenía el eje
    "País" (línea 1168); faltaba **concentración**: `get_concentracion` ahora agrega el eje
    "País" con HHI, mismo tratamiento que Sector (`_agrupar_sobre_total` + bucket "Sin país"
    para que los pesos sumen el 100% real). `diagnostico_engine.score_diversificacion` suma
    "País" a su promedio de ejes, **pero sólo cuando hay ≥ 2 categorías reales** (si nadie
    etiquetó el país, el único bucket "Sin país" daría HHI=1 y hundiría el score sin
    información). Sin cambio de frontend: `Contribucion.tsx` ya renderiza los ejes de
    concentración genéricamente y `Exposicion.tsx` ya mostraba "País". Tests:
    `test_concentracion_pais.py` (2), +2 en `test_diagnostico_engine.py`.
...
- **Tests**: siempre vía docker compose corporativo, workdir raíz:
  ...
  ```
  Baseline actual: **298 pasan, 0 fallan** (176 tras Ola 1-2; +29 con Ola 3 ítem 1:
  ...
  + 1 en `test_market_data.py`; +4 con Ola 5 ítem 11: `test_concentracion_pais.py` (2) + 2 en `test_diagnostico_engine.py`).
